...
class Bypass_3_IO extends Bundle {
    val prd_wb              = Input(Vec(3, UInt(log2Ceil(PREG_NUM).W)))//寄存器名，3对应124路aluout       
    val prj_ex              = Input(Vec(3, UInt(log2Ceil(PREG_NUM).W)))//寄存器名，3对应124路src1       
    val prk_ex              = Input(Vec(3, UInt(log2Ceil(PREG_NUM).W)))//寄存器名，3对应124路src2
    val prf_wdata_wb        = Input(Vec(3, UInt(32.W)))  //写数据，3对应124路
...
class Bypass_3 extends Module {
    ...
    for(i <- 0 until 2){//rd_valid有效，且prd与prj相同，或者rd_valid有效，且prd与prk相同，forward有效，谁相同谁有效谁前递
