...
class Dispatch_IO(n: Int) extends Bundle{
    ...
    val insts_disp_index    = Output(Vec(4, Vec(2, UInt(1.W))))//当前周期需要进入发射队列的指令位于指令组的哪个部分
    val insts_disp_valid    = Output(Vec(4, Vec(2, Bool())))//当前周期需要进入该发射队列的指令掩码
}
//对于功能相同的发射队列，dispatch会比较当前这些发射队列中存在的指令总数，将指令送入总数较少的队列--feng
