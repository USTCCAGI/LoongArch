...
object Decode_Map{
    ...
    val map = Array(
        ...
        COPRDT      -> List(Y, N, Y, ALU_MUL,   RS1_REG, RS2_IMM,  NO_BR,   NO_MEM,   SYST,  RD, RD, IMM_COP, CSR_RD,   NO_EXP),
        COPEXCH     -> List(Y, Y, Y, ALU_MUL,   RS1_REG, RS2_IMM,  NO_BR,   NO_MEM,   SYST,  RD, RD, IMM_COP, CSR_XCHG, NO_EXP),
        COPWR       -> List(Y, N, N, ALU_MUL,   RS1_REG, RS2_IMM,  NO_BR,   NO_MEM,   SYST,  RD, RD, IMM_COP, CSR_WR,   NO_EXP),
        JIRL        -> List(Y, N, Y, ALU_ADD,   RS1_PC,  RS2_IMM,  NO_BR,   NO_MEM,   BR,    RD, RD, IMM_16S, NOT_PRIV, NO_EXP),
        BL          -> List(N, N, Y, ALU_ADD,   RS1_PC,  RS2_IMM,  BR_B,    NO_MEM,   BR,    RD, RD, IMM_26S, NOT_PRIV, NO_EXP),
...
class Decode extends Module {
    ...
    def Imm_Gen(inst: UInt, imm_type: UInt): UInt = {
        ...
        switch(imm_type) {
            ...
            is(IMM_TID)  { imm := 0x40.U }
            is(IMM_ERA)  { imm := 0x6.U }
