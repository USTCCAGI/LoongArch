...
class Predict extends Module{
    ...
    when(update_en){
        when((io.real_jump && gpht_raw_rdata(1) && !lpht_raw_rdata(1)) || (!io.real_jump && !gpht_raw_rdata(1) && lpht_raw_rdata(1))){
            cpht(cmt_col)(cpht_windex) := cpht_raw_rdata + (cpht_raw_rdata =/= 3.U)
        }.elsewhen((io.real_jump && !gpht_raw_rdata(1) && lpht_raw_rdata(1)) || (!io.real_jump && gpht_raw_rdata(1) && !lpht_raw_rdata(1))){
            cpht(cmt_col)(cpht_windex) := cpht_raw_rdata - (cpht_raw_rdata =/= 0.U)
        }.elsewhen((io.real_jump && gpht_raw_rdata(1) && lpht_raw_rdata(1)) || (!io.real_jump && !gpht_raw_rdata(1) && !lpht_raw_rdata(1)) || (io.real_jump && !gpht_raw_rdata(1) && !lpht_raw_rdata(1)) || (!io.real_jump && gpht_raw_rdata(1) && lpht_raw_rdata(1))){
            cpht(cmt_col)(cpht_windex) := cpht_raw_rdata
        }
    }

    val branch_prediction = VecInit.tabulate(2)(i => Mux(cpht_rdata(i)(1), (gpht_rdata(i)(1)) && predict_valid(i) ,(lpht_rdata(i)(1)) && predict_valid(i)))
    ...
    }.elsewhen(btb_rdata(btb_rsel)(pred_hit_index).typ(1) && pred_valid_hit(pred_hit_index)){
        top                 := top + 1.U
        ras(top + 1.U)      := pc(6)(31, 3) ## pred_hit_index ## 0.U(2.W)
    }.elsewhen(btb_rdata(btb_rsel)(pred_hit_index).typ === RET && pred_valid_hit(pred_hit_index)){
        top             := top - 1.U
    }
