...
class Unorder_Issue_Queue[T <: inst_pack_DP_t](n: Int, inst_pack_t: T) extends Module{
    ...
    val issue = (~io.issue_ack.asUInt+1.U)(5,0)  //issue为1的位在下面右移
    //发送内容
    ...
    //输出
    for(i<-0 until n){
        when(qvalid(i) && queue(i).prj_waked && queue(i).prk_waked){
            when(queue(i).prj_wake_by_ld && (queue(i).inst.prj === io.ld_mem_prd) && io.dcache_miss === true.B){
                io.issue_req(i) := false.B
            }
            when(queue(i).prk_wake_by_ld && (queue(i).inst.prk === io.ld_mem_prd) && io.dcache_miss === true.B){
                io.issue_req(i) := false.B
            }
            io.issue_req(i) := true.B
